
import numpy as np
from scipy.stats import entropy

letterFrequency = {
    "E": 12.0,
    "T": 9.10,
    "A": 8.12,
    "O": 7.68,
    "I": 7.31,
    "N": 6.95,
    "S": 6.28,
    "R": 6.02,
    "H": 5.92,
    "D": 4.32,
    "L": 3.98,
    "U": 2.88,
    "C": 2.71,
    "M": 2.61,
    "F": 2.30,
    "Y": 2.11,
    "W": 2.09,
    "G": 2.03,
    "P": 1.82,
    "B": 1.49,
    "V": 1.11,
    "K": 0.69,
    "X": 0.17,
    "Q": 0.11,
    "J": 0.10,
    "Z": 0.07,
}

f_freq = np.zeros(0x100)
for k,f in letterFrequency.items():
    f_freq[ord(k)] = f
f_freq /= np.sum(f_freq)

p = lambda i: "{0:b}".format(i)

C1 = ""
C2 = ""

def as_array(c):
    pairs = np.array([int(hexc, base=0x10) for hexc in c]).reshape((-1,2))
    pairs[:, 0] *= 0x10
    return np.sum(pairs, axis=1)

def decrypt(c):
    a = as_array(c)
    bs = 9999999
    for l in range(1, len(c)):
        keepn = (len(a) // l) * l
        chunks = a[:keepn].reshape((l, -1), order="F") 
        # everything in chunks[0] is encrypted with the first byte of the key
        # everything in chunks[1] with the second... etc
        freqs = np.apply_along_axis(lambda same_keyed: np.bincount(same_keyed, minlength=0x100), axis=1, arr=chunks)
        freqs = freqs / np.sum(freqs, axis=1).reshape((-1,1))
        entropies = entropy(freqs, axis=1)
        score = np.sum(entropies)
        if score < bs:
            print(l, score)
            bs = score


         

if __name__ == "__main__":
    print("First message:\n")
    print(decrypt(C1))
    print("\nSecond message:\n")
    print(decrypt(C2))